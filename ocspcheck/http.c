/*	$Id: http.c,v 1.2 2020/12/13 13:15:10 cvsuser Exp $ */
/*
...
dotlsread(char *buf, size_t sz, const struct http *http)
{
	...
	if (rc == -1)
		warnx("%s: tls_read: %s", http->src.ip, tls_error(http->ctx));
	return rc;
}
...
dotlswrite(const void *buf, size_t sz, const struct http *http)
{
	...
	if (rc == -1)
		warnx("%s: tls_write: %s", http->src.ip, tls_error(http->ctx));
	return rc;
}
...
http_disconnect(struct http *http)
{
	...
	if (http->ctx != NULL) {
		...
		if (rc < 0)
			warnx("%s: tls_close: %s", http->src.ip, tls_error(http->ctx));
...
again:
	...
	if (addrs[cur].family == 4) {
		...
		c = inet_pton(AF_INET, addrs[cur].ip, &((struct sockaddr_in *)&ss)->sin_addr);
		len = sizeof(struct sockaddr_in);
	} else if (addrs[cur].family == 6) {
		family = PF_INET6;
		((struct sockaddr_in6 *)&ss)->sin6_family = AF_INET6;
		((struct sockaddr_in6 *)&ss)->sin6_port = htons(port);
		c = inet_pton(AF_INET6, addrs[cur].ip, &((struct sockaddr_in6 *)&ss)->sin6_addr);
...
http_body_read(const struct http *http, struct httpxfer *trans, size_t *sz)
{
	...
	*sz = 0;
	...
	do {
		...
		pp = recallocarray(trans->bbuf, trans->bbufsz, trans->bbufsz + ssz, 1);
		if (pp == NULL) {
			warn("recallocarray");
			return NULL;
		}
