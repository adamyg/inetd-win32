...
url2host(const char *host, short *port, char **path)
{
	...
	if ((ep = strchr(url, ':')) != NULL) {
		const char *errstr;
		short pp;
		pp = (short)strtonum(ep + 1, 1, SHRT_MAX, &errstr);
		if (errstr != NULL) {
...
parse_ocsp_time(ASN1_GENERALIZEDTIME *gt)
{
	...
	if (ASN1_time_parse(gt->data, gt->length, &tm, V_ASN1_GENERALIZEDTIME) == -1)
		return -1;
...
read_cacerts(const char *file, const char *dir)
{
	...
	if (file != NULL) {
#if defined(_WIN32)
		switch (certstore_cacertificates(store, file)) {
		case -1: // not supported
			break;
		case 0:  // no certificates
...
#if defined(_WIN32)
	if (NULL != (rv = certstore_certificate_chain(file, count))) {
		return rv;
	}
#endif
...
ocsp_request_new_from_cert(const char *cadir, char *file, int nonce)
{
	...
	if ((request->size = i2d_OCSP_REQUEST(request->req, &request->data)) <= 0) {
    ...
    X509_STORE *store, char *host, char *file)
{
	...
	if (OCSP_basic_verify(bresp, request->fullchain, store, OCSP_TRUSTOTHER) != 1) {
	...
	if (status != OCSP_RESPONSE_STATUS_SUCCESSFUL) {
		warnx("OCSP Failure: code %d (%s) from host %s", status, OCSP_response_status_str(status), host);
		goto err;
	}
	...
	if (OCSP_resp_find_status(bresp, cid, &cert_status, &crl_reason, &revtime, &thisupd, &nextupd) != 1) {
	...
	if (this_t > now + JITTER_SEC) {
		warnx("Invalid OCSP reply: this update is in the future at %s", ctime(&this_t));
		goto err;
	}
	...
	if (this_t < now - MAXAGE_SEC) {
		warnx("Invalid OCSP reply: this update is too old %s", ctime(&this_t));
		goto err;
	}
	...
	if (next_t < now - JITTER_SEC) {
		warnx("Invalid OCSP reply: reply has expired at %s", ctime(&next_t));
		goto err;
	}
...
main(int argc, char **argv)
{
	...
	int ch, staplefd = -1, infd = -1, nonce = 1;
	ocsp_request *request = NULL;
	...
	if (outfile != NULL) {
		if (strcmp(outfile, "-") == 0)
			...
			staplefd = open(outfile, O_WRONLY|O_CREAT, S_IWUSR|S_IRUSR|S_IRGRP|S_IROTH);
		if (staplefd < 0)
	...
	if ((castore = read_cacerts(cafile, cadir)) == NULL)
		exit(1);
	if ((request = ocsp_request_new_from_cert(cadir, certfile, nonce)) == NULL)
		exit(1);

	dspew("Built an %zu byte ocsp request\n", request->size);

	if ((host = url2host(request->url, &port, &path)) == NULL)
		errx(1, "Invalid OCSP url %s from %s", request->url, certfile);
	...
	if (infd == -1) {
		 ...
		 */
		hget = http_get(sources, rescount, host, port, path, request->data, request->size);
		if (hget == NULL)
			errx(1, "http_get");
		/*
		...
		if (instaple) {
			while ((nr = read(infd, instaple + instaplesz, OCSP_MAX_RESPONSE_SIZE - instaplesz)) != -1 && nr != 0)
				instaplesz += nr;
		}
		if (instaplesz == 0)
			exit(1);
		 ...
		 */
		OPENSSL_add_all_algorithms_noconf();
		if (!validate_response(instaple, instaplesz, request, castore, host, certfile))
			exit(1);
	}
	...
	if (staplefd >= 0) {
		...
		while (written < instaplesz) {
			w = write(staplefd, instaple + written, instaplesz - written);
			if (w == -1) {
				if (errno != EINTR && errno != EAGAIN)
					err(1, "Write of OCSP response failed");
			} else
