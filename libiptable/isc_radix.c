...
isc_mem_get(isc_mem_t *mctx, size_t size) {
	...
	case sizeof(isc_radix_tree_t):
		break;
	case sizeof(isc_prefix_t):
		++mctx->mem_prefix;
		break;
	case sizeof(isc_radix_node_t):
		++mctx->mem_radix;
		break;
	default:
...
_ref_prefix(isc_mem_t *mctx, isc_prefix_t **target, isc_prefix_t *prefix) {
	...
	if (isc_refcount_current(&prefix->refcount) == 0) {
		isc_result_t ret;
		ret = _new_prefix(mctx, target, prefix->family, &prefix->add, prefix->bitlen);
		return (ret);
	}
...
isc_radix_insert(isc_radix_tree_t *radix, isc_radix_node_t **target,
	...
	assert(RADIX_TREE_VALID(radix->magic));
	assert(target != NULL && *target == NULL);
	assert(prefix != NULL || (source != NULL && source->prefix != NULL));
	assert(prefix == NULL || prefix->bitlen <= radix->maxbits);
	...
	if (radix->head == NULL) {
		...
		if (result != ISC_R_SUCCESS) {
			isc_mem_put(radix->mctx, node, sizeof(isc_radix_node_t));
			return (result);
		}
		node->parent = NULL;
		node->l = node->r = NULL;
		if (source != NULL) {
			...
			for (i = 0; i < RADIX_FAMILIES; i++) {
				if (source->node_num[i] != -1) {
					node->node_num[i] =
						radix->num_added_node + source->node_num[i];
				}
	...
	if (differ_bit == bitlen && node->bit == bitlen) {
		...
		assert(node->data[RADIX_V4] == NULL &&
	...
	if (result != ISC_R_SUCCESS) {
		...
		if (glue != NULL) {
			isc_mem_put(radix->mctx, glue, sizeof(isc_radix_node_t));
		}
		return (result);
	}
	...
	if (source != NULL) {
		...
		for (i = 0; i < RADIX_FAMILIES; i++) {
			int cur = radix->num_added_node;
			if (source->node_num[i] != -1) {
				new_node->node_num[i] = source->node_num[i] + cur;
