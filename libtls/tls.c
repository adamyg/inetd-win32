    ...
    struct tls_keypair *keypair, int required)
{
	...
	if (keypair->cert_mem != NULL) {
#if defined(_WIN32)
		tls_set_errorx(ctx, "certificate memory image not supported");
		goto err;
#else
		...
		if (SSL_CTX_use_certificate_chain_mem(ssl_ctx, keypair->cert_mem, keypair->cert_len) != 1) {
	...
	if (keypair->key_mem != NULL) {
		...
		if ((bio = BIO_new_mem_buf(keypair->key_mem, keypair->key_len)) == NULL) {
			tls_set_errorx(ctx, "failed to create buffer");
			goto err;
		}
		if ((pkey = PEM_read_bio_PrivateKey(bio, NULL, tls_password_cb, NULL)) == NULL) {
...
tls_configure_ssl(struct tls *ctx, SSL_CTX *ssl_ctx)
{
	...
	if (ctx->config->alpn != NULL) {
		if (SSL_CTX_set_alpn_protos(ssl_ctx, ctx->config->alpn, ctx->config->alpn_len) != 0) {
	...
	if (ctx->config->ciphers != NULL) {
		if (SSL_CTX_set_cipher_list(ssl_ctx, ctx->config->ciphers) != 1) {
	...
	if (ctx->config->verify_time == 0) {
		...
		X509_VERIFY_PARAM_set_flags(X509_STORE_get0_param(SSL_CTX_get_cert_store(ssl_ctx)), X509_V_FLAG_NO_CHECK_TIME);
#else
		X509_VERIFY_PARAM_set_flags(ssl_ctx->param, X509_V_FLAG_NO_CHECK_TIME);
#endif
	}
...
tls_configure_ssl_verify(struct tls *ctx, SSL_CTX *ssl_ctx, int verify)
{
...
#else
	if (ctx->config->ca_path == NULL) {
#endif
		if (tls_config_load_file(&ctx->error, "CA", tls_default_ca_cert_file(), &ca_mem, &ca_len) != 0)
			goto err;
		ca_free = ca_mem;
	}
...
#endif
            if (SSL_CTX_load_verify_locations(ssl_ctx, NULL, ctx->config->ca_path) != 1) {
	...
	if (crl_mem != NULL) {
		...
		if ((xis = PEM_X509_INFO_read_bio(bio, NULL, tls_password_cb, NULL)) == NULL) {
...
#if OPENSSL_VERSION_NUMBER >= 0x10001000L
		X509_VERIFY_PARAM_set_flags(X509_STORE_get0_param(store), X509_V_FLAG_CRL_CHECK | X509_V_FLAG_CRL_CHECK_ALL);
#else
		X509_VERIFY_PARAM_set_flags(store->param, X509_V_FLAG_CRL_CHECK | X509_V_FLAG_CRL_CHECK_ALL);
#endif
	}
