...
tls_ocsp_get_certid(X509 *main_cert, STACK_OF(X509) *extra_certs, SSL_CTX *ssl_ctx)
{
	...
	{ 	X509_STORE_CTX *storectx = NULL;
		...
		if (NULL == (storectx = X509_STORE_CTX_new()) ||
				X509_STORE_CTX_init(storectx, store, main_cert, extra_certs) != 1) {
			if (storectx) {
				X509_STORE_CTX_free(storectx);
			}
			return NULL;
		}
		if (NULL != (tmpobj = X509_OBJECT_new())) {
			if (X509_STORE_get_by_subject(storectx, X509_LU_X509, issuer_name, tmpobj) == 1) {
...
tls_ocsp_stapling_cb(SSL *ssl, void *arg)
{
	...
	memcpy(ocsp_staple, ctx->keypair->ocsp_staple, ctx->keypair->ocsp_staple_len);

	if (SSL_set_tlsext_status_ocsp_resp(ctx->ssl_conn, ocsp_staple, ctx->keypair->ocsp_staple_len) != 1)
		goto err;
