...
#if defined(_WIN32) && !defined(HAVE_FREEZERO)
...
tls_config_set_ecdhecurves(struct tls_config *config, const char *curves)
{
	...
	while ((p = strsep(&q, ",:")) != NULL) {
		...
		if ((curves_new = reallocarray(curves_list, curves_num + 1, sizeof(int))) == NULL) {
			tls_config_set_errorx(config, "out of memory");
			goto err;
		}
...
{
	return tls_keypair_set_key_file(config->keypair, &config->error, key_file);
}

int
tls_config_set_key_mem(struct tls_config *config, const uint8_t *key, size_t len)
{
	return tls_keypair_set_key_mem(config->keypair, &config->error, key, len);
}
...
tls_config_set_keypair_file(struct tls_config *config,
    const char *cert_file, const char *key_file)
{
	return tls_config_set_keypair_file_internal(config, cert_file, key_file, NULL);
}
...
#else
#pragma message("TODO: test file ownership ...")
#endif
    ...
    unsigned char *key, size_t keylen)
{
	...
	if (TLS_TICKET_KEY_SIZE != keylen ||
		...
		tls_config_set_errorx(config, "wrong amount of ticket key data");
		return (-1);
	}
...
tls_config_ticket_autorekey(struct tls_config *config)
{
	...
	rv = tls_config_add_ticket_key(config, config->ticket_keyrev++, key, sizeof(key));
	config->ticket_autorekey = 1;
	return (rv);
}
