...
int
tls_hex_string(const unsigned char *in, size_t inlen, char **out, size_t *outlen)
{
...
tls_get_peer_cert_info(struct tls *ctx)
{
	...
	if (tls_get_peer_cert_issuer(ctx, &ctx->conninfo->issuer) == -1)
		goto err;
	if (tls_get_peer_cert_times(ctx, &ctx->conninfo->notbefore, &ctx->conninfo->notafter) == -1)
		goto err;
