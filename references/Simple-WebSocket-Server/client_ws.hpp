...
namespace SimpleWeb {
  ...
  public:
    ...
    public:
      ...
      void send(const std::shared_ptr<OutMessage> &out_message, std::function<void(const error_code &)> callback = nullptr, unsigned char fin_rsv_opcode = 129) {
        ...
        if(length >= 126) {
          ...
          if(length > 0xffff) {
            num_bytes = 8;
            out_header_and_message->put(static_cast<unsigned char>(127 + 128));
          }
          else {
            num_bytes = 2;
            out_header_and_message->put(static_cast<unsigned char>(126 + 128));
          }
          ...
          out_header_and_message->put(static_cast<unsigned char>(length + 128));
    ...
    std::function<void(std::shared_ptr<Connection>, const error_code &, const asio::ip::tcp::resolver::iterator next)> on_attempt; //APY
    std::function<void(std::shared_ptr<Connection>)> on_open;
    std::function<bool(std::shared_ptr<Connection>, const std::string &location)> on_redirect; //APY
    ...
    void set_header(const char *tag, const char *value) { //APY
      if (tag && *tag) {
        auto it = config.header.find(tag);
        if (it != config.header.end()) {
          if (value) {
            config.header.erase(it);
          } else {
            it->second = value;
          }
        } else {
          if (value) {
            config.header.emplace(tag, value);
          }
        }
      }
    }
  ...
  protected:
    ...
    void upgrade(const std::shared_ptr<Connection> &connection) {
      ...
      if (config.protocol.length()) //APY
        ostream << "Sec-WebSocket-Protocol: " << config.protocol << "\r\n";
        ...
        if(!ec) {
          ...
          asio::async_read_until(*connection->socket, connection->in_message->streambuf, "\r\n\r\n", [this, connection, nonce_base64](const error_code &ec, std::size_t bytes_transferred) {
            ...
            if(!ec) {
              ...
              if(connection->status_code.compare(0, 4, "101 ") != 0) { // 101 Switching Protocols, APY
                if(connection->status_code.compare(0, 4, "302 ") == 0) { // 302 Found
                  CaseInsensitiveMultimap::const_iterator location_it = connection->header.find("Location");
                  if(location_it != connection->header.end()) {
                    if(connection_redirect(connection, location_it->second)) {
                      connection->send_close(1001);
                      this->connection_close(connection, 1001, "Redirection");
                      return;
                    }
                  }
                }
    ...
    void connection_attempt(const std::shared_ptr<Connection> &connection, const error_code &ec, const asio::ip::tcp::resolver::iterator next) const { //APY
      if(on_attempt)
        on_attempt(connection, ec, next);
    }
    ...
    bool connection_redirect(const std::shared_ptr<Connection> &connection, const std::string &location) {
      if(! location.empty()) {
        if(on_redirect) {
          return on_redirect(connection, location);
        }
      }
      return false;
    }
