...
#if defined(_WIN32) /*APY*/
#include "crypto_win32.hpp"
#else
#include "crypto.hpp"
#endif
#include "mutex.hpp"
...
namespace SimpleWeb {
  ...
  public:
    ...
    public:
      std::function<bool(std::shared_ptr<Connection>)> on_accept; /*APY*/
  ...
  protected:
    void close_connections() { /*APY*/
      for(auto &pair : endpoint) {
        SimpleWeb::LockGuard lock(pair.second.connections_mutex);
    ...
    void write_handshake(const std::shared_ptr<Connection> &connection) {
      for(auto &regex_endpoint : endpoint) {
        ...
        if(regex::regex_match(connection->path, path_match, regex_endpoint.first)) {
          ...
          //  The value of this header field MUST be a nonce consisting of a randomly selected 16-byte value
          //  that has been base64-encoded (see Section 4 of [RFC4648]).
          ...
          auto version_it = connection->header.find("Sec-WebSocket-Version"); /*APY*/
          if(key_it == connection->header.end())
            status_code = StatusCode::client_error_upgrade_required;
          else if(version_it == connection->header.end() || 13 != std::atoi(version_it->second.c_str()))
            status_code = StatusCode::client_error_upgrade_required;
          else {
  ...
  protected:
    ...
    void accept(asio::detail::socket_type fd, const asio::ip::tcp &protocol = asio::ip::tcp::v4()) { /*APY*/
      std::shared_ptr<Connection> connection(new Connection(handler_runner, config.timeout_idle, *io_service));

      // assume ownership of the external socket descriptor.
      connection->socket->assign(protocol, fd);
